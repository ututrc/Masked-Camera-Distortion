...
#include "UnityCustomStandardConfig.cginc"
#include "UnityCustomStandardInput.cginc"
#include "UnityPBSLighting.cginc"
#include "UnityCustomStandardUtils.cginc"
#include "UnityCustomStandardBRDF.cginc"
...
struct VertexOutputForwardBase
{
	...
	#ifdef CUSTOM_DEPTH_BASED_FADE
		float4	cameraSpacePos			: TEXCOORD9;
	#endif
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
	...
	half4 outputColor = OutputForward (c, s.alpha);

	#ifdef CUSTOM_DEPTH_BASED_FADE
		float maxCameraDepth = 30.0;

		float depthScale = 1.0;
		if (i.cameraSpacePos.z > _DepthBasedFadeRangeStart)
		{
			half rangeLenght = _DepthBasedFadeRangeEnd - _DepthBasedFadeRangeStart;
			if (rangeLenght > 0.0)
				depthScale = clamp((_DepthBasedFadeRangeEnd - i.cameraSpacePos.z) / rangeLenght, 0.0, 1.0);
		}
		outputColor.a = depthScale;
	#endif
